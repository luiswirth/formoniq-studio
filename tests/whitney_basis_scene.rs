...
/// reduction is taken against $dim partial K = 2$, not against 3:
///
/// - grade 0 traces to a 0-form on $partial K$: reduced grade 0, a density;
/// - grade 1 traces to a 1-form on $partial K$: reduced grade $min(1, 1) = 1$, a
///   line field, the arrows, and the only grade that gets them here;
/// - grade 2 traces to $partial K$'s top form: reduced grade $min(2, 0) = 0$, a
...
/// - grade 3 does not trace at all ($C^3(partial K) = 0$), so it keeps the
